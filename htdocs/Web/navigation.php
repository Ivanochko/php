<div id="nav">
    <div class="nav-elem">
        <div class="nav-back theme-button" id="theme-button" title="Натисніть, щоб змінити тему"></div>
    </div>
    <div class="nav-elem">
        <div class="nav-back to-main"><a target="_blank" href="http://ivanochko.great-site.net/Web/" class="nav-href to-main"></a></div>
    </div>
    <div class="nav-elem">
        <div class="nav-back mail-me"><a target="_blank" href="mailto:vasa.ivanochko.17@gmail.com" class="mail-me nav-href"></a></div>
    </div>
    <div class="nav-elem">
        <div class="nav-back telegram-me"><a target="_blank" href="" class="telegram-me nav-href"></a></div>
    </div>
    <div class="nav-elem">
        <div class="nav-back my-git"><a target="_blank" href="http://github.com/Ivanochko" class="my-git nav-href"></a></div>
    </div>
</div>